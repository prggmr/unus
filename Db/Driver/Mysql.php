...
class Unus_Db_Driver_Mysql implements Unus_Db_Driver_Interface
{
    /**
     ...
     *
     * @param  mixed    id      Value of primary key in database to update | Array of fields => values
     * @param  array    fields  Fields -> Values for query delete
     *
     * @return string
     */
     ...
     * @return this
     */
    public function limit($start = 0, $limit = 30, $offset = 0)
    {
    ...
    public function setModel($model)
    {
		...
		if (null == $this->_model) {
			throw new Unus_Db_Driver_Exception(sprintf('Call to unknown model (%s)', $model));
		}
        return $this;
    }
    ...
    public function build()
    {
        ...
        if ($this->_where != null) {
            ...
            foreach ($this->_where as $k => $v) {
                ...
                $this->_where[$k] = array('value' => $v);

                if ($typehint) {
                    $this->_where[$k]['operator'] = $hinting;
                    if ($novalue) {
						echo 'setting now value';
        ...
        if (null != $this->_where) {
            ...
            foreach ($this->_where as $k => $v) {
                if (array_key_exists('noval', $v) && $v['noval'] == true) {
                    ...
                    $sql .= ($a >= 1) ? ' AND `'.$k.'` '.$v['operator'].''  :  ' `'.$k.'` '.$v['operator'].'';
                } else {
                    $sql .= ($a >= 1) ? ' AND `'.$k.'` '.$v['operator'].' :'.$k.'' :  ' `'.$k.'` '.$v['operator'].' :'.$k.'';
                }
                $a++;
            }
        }
    ...
    private function _addField($name, Unus_Model_Table $model)
    {
        ...
        if (strpos($name, ':') !== false) {
            ...
            $name = $str[0];
            $oldModel = $model;
            $model = Unus_Model::getInstance()->getData('registry')->getModel($str[1], true);
        ...
        } elseif(!$this->_fieldExists($name, $model)) {
            /**
            ...
            foreach ($model->getData('fields') as $k => $v) {
                if ($v['type'] == Unus_Model_Table::FOREIGN) {
                    if ($v['options']['use'] != false) {
                        $temp = Unus_Model::getInstance()->getData('registry')->getModel($v['options']['use']);
     ...
     * @return mixed
     */
    public function select($fields, $build = true)
    {
		if (is_array($fields)) {
			$this->_select = $fields;
		} else {
    ...
    private function _fieldExists($field, Unus_Model_Table $model, $exception = true)
    {
        if (!$model->fieldExists($field) && $field != '*') {
            ...
            if ($exception) {
                throw new Unus_Db_Driver_Mysql_Exception('Field [ '.$field.' ] in model [ '.$model->getData('id').' ] is not avaliable; avaliable fields: '.$av);
            }
