...
class Unus_Form_Validator_Email extends Unus_Form_Validator_Abstract
{
	...
	public function jqueryValidate()
	{
        ...
        $validate->addElement($this->getData('element_name'), array('required' => 'true', 'email' => 'true'));
        $validate->addMessage($this->getData('element_name'), array(
																	'required' => __(Unus_Validate_Email::STRING_EMPTY),
																	'email' => __(Unus_Validate_Email::ERROR_REGEX)));
	}
	...
	public function isValid($val)
	{
		...
		$email = new Unus_Validate_Email();
		
		if ($this->_checkDns) {
			$email->checkDns(true);
		}
		
		if (!$email->isValid($val)) {
			$errors = $email->getMessages();
